...
export default function Home(){
 ...
 useEffect(()=>{
   try {
     const savedPage=localStorage.getItem("tradingwig-page") as "watchlist"|"chart"|null;
     const savedCharts=localStorage.getItem("tradingwig-charts");
     const savedLayout=localStorage.getItem("tradingwig-layout");
     if(savedPage) setPage(savedPage);
     if(savedCharts) setCharts(JSON.parse(savedCharts));
     if(savedLayout) setLayout(Math.max(1,Math.min(4,Number(savedLayout)||1)));
   } catch {}
   setRestored(true);
 },[]);
 useEffect(()=>{ if(restored) localStorage.setItem("tradingwig-page",page); },[page,restored]);
 useEffect(()=>{ if(restored) localStorage.setItem("tradingwig-charts",JSON.stringify(charts)); },[charts,restored]);
 useEffect(()=>{ if(restored) localStorage.setItem("tradingwig-layout",String(layout)); },[layout,restored]);
