...
function chartUrl(symbol: string, interval: string, index: number) {
  const params = new URLSearchParams({
    frameElementId: `tradingview_widget_${index}`, symbol: `OANDA:${symbol}`, interval,
    hide_side_toolbar: "1", allow_symbol_change: "1", save_image: "1", toolbarbg: "#ffffff", studies: "[]",
    theme: "light", style: "1", timezone: "Etc/UTC", withdateranges: "1", hideideas: "1", hide_volume: "0",
    enable_publishing: "0", hide_top_toolbar: "0", hide_legend: "0"
  });
...
function DrawingOverlay({ tool, drawings, onAdd }: { tool: Tool; drawings: Drawing[]; onAdd: (d: Drawing) => void }) {
  ...
  const down = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    const p = point(e);
    if (tool === "horizontal") {
      onAdd({ type: tool, a: p, b: { x: rWidth(e), y: p.y } });
      return;
    }
    setStart(p);
  };
  const up = (e: React.PointerEvent<SVGSVGElement>) => {
    if (!tool || !start || tool === "horizontal") return;
    onAdd({ type: tool, a: start, b: point(e) });
    setStart(null);
  };
  const rWidth = (e: React.PointerEvent<SVGSVGElement>) => e.currentTarget.getBoundingClientRect().width;

  return (
    <svg className={`drawing-overlay ${active ? "drawing-active" : ""}`} onPointerDown={down} onPointerUp={up}>
      {drawings.map((d, i) => {
        if (d.type === "horizontal") return <line key={i} x1={0} y1={d.a.y} x2="100%" y2={d.a.y} className="drawing-line" />;
        if (d.type === "rectangle") return <rect key={i} x={Math.min(d.a.x,d.b.x)} y={Math.min(d.a.y,d.b.y)} width={Math.abs(d.b.x-d.a.x)} height={Math.abs(d.b.y-d.a.y)} className="drawing-rect" />;
        if (d.type === "fib") {
          const levels = [0, .236, .382, .5, .618, 1];
          const y1 = d.a.y, y2 = d.b.y;
          return <g key={i}>{levels.map((n,j)=><line key={j} x1={Math.min(d.a.x,d.b.x)} x2={Math.max(d.a.x,d.b.x)} y1={y1+(y2-y1)*n} y2={y1+(y2-y1)*n} className="fib-line" />)}</g>;
        }
        return <line key={i} x1={d.a.x} y1={d.a.y} x2={d.b.x} y2={d.b.y} className="drawing-line" />;
      })}
    </svg>
  );
}
...
export default function Home() {
  ...
  const [drawings, setDrawings] = useState<Record<number, Drawing[]>>({});
  const visibleCharts = useMemo(() => charts.slice(0, layout), [charts, layout]);

  function openSymbol(symbol: string) { setCharts(c => c.map((x,i) => i===0 ? {...x,symbol} : x)); setLayout(1); setPage("chart"); }
  function setInterval(index:number, interval:string, label:string) { setCharts(c => c.map((x,i) => i===index ? {...x,interval,label} : x)); }
  function chooseTool(next: Tool) { setTool(next); setToolsOpen(false); }
  ...
  if (page === "watchlist") return (
    <main className="mobile-app watchlist-page">
      ...
      <div className="watchlist-tabs"><button className="hamburger">☰</button><button className="list-tab active">Watchlist</button><button className="add-list">＋ Add list</button></div>
      <section className="quotes">{symbols.map(item => <button className="quote-row" key={item.symbol} onClick={()=>openSymbol(item.symbol)}>
        ...
        <span className="quote-values"><strong>{item.price}</strong><small className={item.positive?"up":"down"}>{item.change} {item.percent}</small></span>
      </button>)}</section>
  ...
  return <main className="mobile-app chart-page">
    <section className={`chart-stage layout-${layout}`}>{visibleCharts.map((chart,index)=><article className="chart-card" key={index}><iframe className="chart-frame" src={chartUrl(chart.symbol,chart.interval,index)} title={`${chart.symbol} ${chart.label} TradingView chart`} allowFullScreen /><DrawingOverlay tool={index === 0 ? tool : null} drawings={drawings[index] || []} onAdd={d=>setDrawings(v=>({...v,[index]:[...(v[index]||[]),d]}))} /></article>)}</section>

    <section className="chart-controls">
      <div className="chart-symbol-strip"><button className="control-symbol" onClick={()=>setPage("watchlist")}>‹</button><button className="control-symbol selected">{charts[0].symbol}</button>{intervals.map(item=><button key={item.interval} className={`control-timeframe ${charts[0].interval===item.interval?"selected":""}`} onClick={()=>setInterval(0,item.interval,item.label)}>{item.label}</button>)}</div>
      <div className="tool-row">
        <button className={`tool-button ${toolsOpen || tool ? "active" : ""}`} onClick={()=>{setToolsOpen(!toolsOpen);setLayoutOpen(false)}} aria-label="Drawing tools">✎</button>
        <button className="tool-button" aria-label="Magnet">⌁</button>
        <button className="tool-button more" onClick={()=>setTool(null)} aria-label="Clear drawing mode">•••</button>
        <button className="tool-button" onClick={()=>setDrawings({})} aria-label="Clear drawings">↶</button>
      </div>
      {toolsOpen && <div className="tools-menu"><button onClick={()=>chooseTool("trend")}>Trend Line</button><button onClick={()=>chooseTool("horizontal")}>Horizontal Line</button><button onClick={()=>chooseTool("rectangle")}>Rectangle</button><button onClick={()=>chooseTool("fib")}>Fib Retracement</button></div>}
      ...
      {layoutOpen && <div className="layout-menu">{[1,2,4].map(count=><button key={count} className={layout===count?"selected":""} onClick={()=>{setLayout(count);setLayoutOpen(false)}}>{count} Chart{count>1?"s":""}</button>)}</div>}
    </section>
