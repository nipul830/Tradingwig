...
function chartUrl(symbol: string, interval: string, index: number) {
  const params = new URLSearchParams({ frameElementId: `tradingview_widget_${index}`, symbol: `OANDA:${symbol}`, interval,
    hide_side_toolbar: "1", allow_symbol_change: "1", save_image: "1", toolbarbg: "#ffffff", studies: "[]",
    theme: "light", style: "1", timezone: "Etc/UTC", withdateranges: "1", hideideas: "1", hide_volume: "1",
...
export default function Home() {
  ...
  return <main className="mobile-app chart-page">
    <section className={`chart-stage layout-${layout}`}>{visibleCharts.map((chart,index)=><article className="chart-card" key={index}><iframe className="chart-frame" src={chartUrl(chart.symbol,chart.interval,index)} title={`${chart.symbol} ${chart.label} TradingView chart`} allowFullScreen /></article>)}</section>
    <section className="chart-controls">
      ...
      <div className="tool-row"><button className={`tool-button ${toolsOpen?"active":""}`} onClick={()=>{setToolsOpen(!toolsOpen);setLayoutOpen(false)}} aria-label="Drawing tools">✎</button><button className="tool-button">⌁</button><button className="tool-button more">•••</button><button className="tool-button">↶</button></div>
      {toolsOpen && <div className="tools-menu"><button>Trend Line</button><button>Horizontal Line</button><button>Rectangle</button><button>Fib Retracement</button></div>}
